...
def test_batch_size_caps_are_declared():
    ...
    for cls in (Qwen2_5VL, Qwen3VL, InternVL3_8B, LlavaNextVideo):
...
def test_render_question_is_shared_across_backends():
    ...
    row = make_row("q1", "vidA")
    prompts = {
        cls.render_question(row)
        for cls in (VLM, InternVL3_8B, LlavaNextVideo, Qwen2_5VL, Qwen3VL, VideoLlama3)
    }
